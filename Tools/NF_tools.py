...
# Define NF class
class NF(nn.Module):
    def __init__(self, device, NF_in, NF_hidden_unit, NF_hidden_layer, NF_block, act_s, act_t, BN ):
        super().__init__()

        # self parameters
        self.device             = device
        self.NF_in              = NF_in            # input size
        ...
        for nBlock in range(self.NF_block):
            ...
            if self.BN == True:
                module_list += self.BN * [Batch_Norm(self.NF_in, self.device) ]
    ...
    # Note: we are considering a composition of the independent univariate Gaussian, hence base_var is a vector
    # Note: the original code does not have sqrt, its fine since we only consider standard gaussian
    @property
    def base_dist(self):
        return D.Normal(self.Base_mean, torch.sqrt(self.Base_var)) 
    ...
    def LogProb(self, y):
        ...
        # NaN can exist during training and inference, replace the sample produces NaN with low likelihood
        if (torch.isnan(z).any()) or (torch.isnan(sum_log_det_J).any()):
            print('NaN exists! replace it with a low-likelihood number! ')
            # replace NaN with a fair large number for z so it yields a super low likelihood
            z             = torch.where(torch.isnan(z), torch.full_like(z, 1e16), z)
            # replace NaN with a fair smaller number for likelihood 
            sum_log_det_J = torch.where(torch.isnan(sum_log_det_J), torch.full_like(sum_log_det_J, -1e16), sum_log_det_J)

        # likelihood of z, the gaussian likelihood
        z_likelihood = torch.sum(self.base_dist.log_prob(z), dim=1).reshape((-1,1))

        return  z, z_likelihood + sum_log_det_J # c.f. equation 3 of Dinh

# batch norm of normalizing flows, applied to the end of each coupling layer
class Batch_Norm(nn.Module):

    def __init__(self, input_size, device, momentum = 0.9, eps = 1e-5):
        super().__init__()
    ...
    def forward(self, x):
        ...
        return y, torch.sum(log_abs_det_jacobian)*torch.ones(len(y),1, device=self.device)
    ...
    def inverse(self, y):
        ...
        return x, torch.sum(log_abs_det_jacobian)*torch.ones(len(x),1, device=self.device)
