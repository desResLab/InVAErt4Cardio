...
class simulator_cvsim6:
	...
	def flow_update(self, P_array):
		...
		# init
		Q_li_t = np.zeros(sol_num)
		Q_lo_t = np.zeros(sol_num)
		Q_a_t  = np.zeros(sol_num)
		Q_ri_t = np.zeros(sol_num)
		Q_ro_t = np.zeros(sol_num)
		Q_pv_t = np.zeros(sol_num)
	...
	def volume_update(self, t_array, P_array):
		...
		C_l   =  np.zeros(len(t_array))
		C_r   =  np.zeros(len(t_array))
	...
	def post_process(self, sol, save_traj=False):
		...
		t_sol = sol.t
		P_sol = sol.y # [P_l, P_a, P_v, P_r, P_pa, P_pv] 
		...
		R_svr = ( get_average(t_sol[Tp:], P_sol[1,Tp:]) - P_cvp * self.mmHg2Barye ) / CO / self.mmHg2Barye

		# 15: Pulmonary vascular resistance (mmHg.min/L) (HRU)
		R_pvr = ( get_average(t_sol[Tp:], P_sol[4,Tp:]) - Pw * self.mmHg2Barye ) / CO / self.mmHg2Barye
		# *************************************************************************************************** #

		# group output set
		output_set = np.array([Pa_sys, Pa_dia, Pr_sys, Pr_dia, Ppa_sys, Ppa_dia, Pr_edp, Pw, P_cvp,\
		...
		if save_traj == False:
			...
			return output_set, t_sol, P_sol, \
					np.stack([Q_li_t, Q_lo_t, Q_a_t, Q_ri_t, Q_ro_t, Q_pv_t]), \
					np.stack([V_lv_S, V_a_S, V_v_S, V_rv_S, V_pa_S, V_pv_S])
	# --------------------------------------------------------------------------------------#
	...
	#        num_t: total number of steps saved
	#        rtol:  relative tolerance of the implicit method
	#        atol:  absolute tolerance of the implicit method
	# Outputs:
	#        interested output quantities if save_traj == False
	#        interested output quantities plus solution trajectories if save_traj == True
	def solve(self, input_para, save_traj = False, num_t = 2400, rtol = 1e-6, atol = 1e-8):
		...
		t_eval = np.linspace(0., self.T_total_cycle, self.num_t) 

		# solve the system
		# Note: solve a little bit longer, this is to make sure t_eval's end point is always reachable
		sol    = solve_ivp(self.f, [0., self.T_total_cycle + 0.1], P0, method= 'Radau', t_eval = t_eval, \
																						rtol=rtol, atol = atol)
		# ------------------------------------------------------------------------------------------------------- #
